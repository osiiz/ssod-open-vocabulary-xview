...
class RexOmniWrapper:
    ...
    def _inference_batch(
        ...
        **kwargs,
        ...
        if self.model_type == "vllm":
            if return_scores:
                raise NotImplementedError(
                    "return_scores=True solo soportado con backend='transformers'"
                )
            batch_outputs, batch_generation_info = self._generate_vllm_batch(
                batch_messages
            )
            batch_token_data = None
        else:
            (
                batch_outputs,
                ...
                batch_messages, images, capture_scores=return_scores
            )
        ...
        for i in range(batch_size):
            ...
            # Parse predictions
            if (
                batch_token_data is not None
                and task.value == "detection"
                and batch_token_data[i] is not None
            ):
                from .parser import parse_prediction_with_scores

                extracted_predictions = parse_prediction_with_scores(
                    gen_ids=batch_token_data[i]["gen_ids"],
                    scores=batch_token_data[i]["scores"],
                    w=w,
                    h=h,
    ...
    def _generate_transformers_batch(
    ...
    ) -> Tuple[List[str], List[Dict], Optional[List[Dict]]]:
        """Generate using Transformers model for batch processing.

        Si capture_scores=True, devuelve ademas batch_token_data: lista de dicts
        por imagen con keys 'gen_ids' (tensor 1D) y 'scores' (tuple de tensores
        (vocab_size,) por paso de generacion). Usado para extraer scores de
        deteccion desde los logits.
        """
        ...
        with torch.no_grad():
            if capture_scores:
                ...
                raw_scores = gen_output.scores  # tuple de (B, vocab_size) por step
            else:
                generated_ids = self.model.generate(**inputs, **generation_kwargs)
                raw_scores = None
        ...
        # Si se pidieron scores, construir batch_token_data por imagen.
        # Sliciamos scores[step] (B, vocab) -> (vocab,) por imagen para que el
        # parser solo vea su porcion. raw_scores[step].shape = (B, vocab_size).
        batch_token_data = None
        if capture_scores and raw_scores is not None:
            batch_token_data = []
            for i, output_ids in enumerate(generated_ids_trimmed):
                gen_len = len(output_ids)
                # Cada step k corresponde al token output_ids[k]; raw_scores[k]
                # es (B, vocab) de los logits ANTES de generar ese token.
                # Slice por imagen y por longitud generada (max_new_tokens es global,
                # pero output_ids puede ser mas corto si EOS).
                per_image_scores = tuple(
                    raw_scores[k][i].detach().cpu() for k in range(gen_len)
                )
                batch_token_data.append(
                    {
                        "gen_ids": output_ids.detach().cpu(),
                        "scores": per_image_scores,
                    }
                )
            # Liberar memoria del tensor grande
            del raw_scores

        return batch_outputs, batch_generation_info, batch_token_data
